...
fn add_op() -> Result<()> {
    ...
    assert_eq!(result.array_shape()?, hodu_xla::ArrayShape::new::<f32>(vec![2]));
...
fn sum_op() -> Result<()> {
    ...
    let input = hodu_xla::Literal::vec1(&[4.2f32, 1.337f32]);
    let result = sum.execute::<hodu_xla::Literal>(&[input])?;
    let result = result[0][0].to_literal_sync()?;
...
fn tuple_op() -> Result<()> {
    ...
    let x = hodu_xla::Literal::scalar(3.1f32);
    let y = hodu_xla::Literal::vec1(&[4.2f32, 1.337f32]);
    let result = tuple.execute::<hodu_xla::Literal>(&[x, y])?;

    assert_eq!(result[0].len(), 2);
    let result0 = result[0][0].to_literal_sync()?;
    let result1 = result[0][1].to_literal_sync()?;
    assert_eq!(result0.to_vec::<f32>()?, [3.1]);
    assert_eq!(result1.to_vec::<f32>()?, [4.2, 1.337]);
    Ok(())
}
...
fn tuple_literal() -> Result<()> {
    let x = hodu_xla::Literal::scalar(3.1f32);
    let y = hodu_xla::Literal::vec1(&[4.2f32, 1.337f32]);
    let result = hodu_xla::Literal::tuple(vec![x, y]);
